...
const getAllGenres = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      message: "Error getting all genres",
      error: error.errors,
    });
  }
};
...
const addNewGenre = async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ success: true, message: `${genre.name} was added`, data: genre });
  } catch (error) {
    return res.status(500).json({
      success: false,
...
const getBooksByGenreName = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: `Books by genre ${genreName} returned`,
      count: books.length,
      data: books,
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
      message: "Error getting books by genre",
      error: error.errors,
    });
  }
};
